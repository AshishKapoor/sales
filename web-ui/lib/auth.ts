import { SALES_BASE_URL } from "@/client/http-sales-client";
...
export async function refreshAccessToken() {
  ...
  try {
    const response = await fetch(`${SALES_BASE_URL}/api/token/refresh/`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function getUser() {
  try {
    ...
    const response = await fetch(`${SALES_BASE_URL}/api/v1/me/`, {
      method: "GET",
      headers: {
